        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[101] grid w-[calc(100vw-2rem)] max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 rounded-[var(--px-radius)] border border-[color:var(--px-border)] bg-[color:var(--px-surface)] p-6 shadow-lg duration-200 focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          <div className="flex flex-col gap-2">
            ...
            <Dialog.Description className="text-sm text-[color:var(--px-text-muted)]">
              {explanation ||
                "This feature is being prepared for a future perX update."}
              {featureName && (
          ...
          <div className="mt-4 flex justify-end">
            <Dialog.Close asChild>
              <button className="rounded-[var(--px-radius-sm)] bg-[color:var(--px-muted)] px-4 py-2 text-sm font-semibold text-[color:var(--px-text)] hover:bg-[color:var(--px-border)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--px-focus)]">
                Close
              </button>
            </Dialog.Close>
          </div>
          <Dialog.Close className="absolute right-4 top-4 rounded-[var(--px-radius-sm)] opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[color:var(--px-primary)] focus:ring-offset-2 disabled:pointer-events-none">
